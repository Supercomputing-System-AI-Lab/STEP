...
class EngineCore:
    ...
    def tensor_to_list_inputs(self, inputs):
        json_inputs = []
        for item in inputs:  # item: Dict[str, Tensor]
            d = {}
    ...
    def _insert_eos(self, model_output, request_idx, stop_reason: str | None = None):
        eos_tokenid = self.model_executor.model_config.hf_config.eos_token_id    
        if stop_reason:
            self.scheduler.mark_stop_reason(request_idx, stop_reason)
        for i in request_idx:
            if i in model_output.req_id_to_index.keys():
                model_output.sampled_token_ids[model_output.req_id_to_index[i]] = [eos_tokenid]
        return
    
    def _lowest_score_traces(
        self, limit: int = 2, candidate_ids: set[str] | None = None
    ) -> list[str]:
        scored: list[tuple[float, str]] = []
        for rid, scores in self.trace_scores.items():
            if (
                not scores
                or rid in self.pending_low_score_stops
                or self.trace_token_counts.get(rid, 0) < self.min_tokens_before_prune
                or (candidate_ids is not None and rid not in candidate_ids)
            ):
                continue
            avg_score = sum(scores) / len(scores)
            scored.append((avg_score, rid))
