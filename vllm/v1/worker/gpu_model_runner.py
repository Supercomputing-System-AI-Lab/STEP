...
class GPUModelRunner(
    ...
    def _bookkeeping_sync(
        ...
        if (
            ...
            captured_req_ids: set[str] = set()
            offset = 0
            for rid in req_ids:
                count = scheduler_output.num_scheduled_tokens.get(rid, 0)
                if count <= 0:
                    continue
                if rid in self._capture_hidden_states_next_forward:
                    cached[rid] = hidden_states[offset : offset + count].detach()
                    captured_req_ids.add(rid)
                offset += count
            if cached:
                self._last_hidden_states_by_req.clear()
                self._last_hidden_states_by_req.update(cached)
            self._capture_hidden_states_next_forward.difference_update(
                captured_req_ids
            )
    ...
    def sample_tokens(
        ...
        # total_tokens = self.requests[rid].num_tokens  # prompt+已生成
        # filename = f"/work/nvme/bcjw/bhuang4/hiddenstate_verification/hs_{rid}_tokens{total_tokens}.pt"
        # torch.save(cached[rid].cpu(), filename)
        ...
        if self.vllm_config.STEP_config.enable and valid_sampled_token_ids:
            ...
            for rid, idx in req_id_to_index_output_copy.items():
                if idx >= len(valid_sampled_token_ids):
                    continue
                tokens = valid_sampled_token_ids[idx]
            ...
            if trigger_req_ids:
                self._capture_hidden_states_next_forward.update(trigger_req_ids)
