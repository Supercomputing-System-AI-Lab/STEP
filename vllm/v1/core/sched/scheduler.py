...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        try:
            ...
            raise ValueError(
                f"Unknown scheduling policy: {requested_policy}"
            ) from e
        ...
        # online trace score
        self.trace_scores: dict[str, float] = {}
        self.pending_pruned_outputs = defaultdict(list)

    # online updating 
    def update_trace_scores(self, scores: dict[str, float]) -> None:
        for rid, score in scores.items():
            self.trace_scores[rid] = score
            if req := self.requests.get(rid):
                req.trace_avg_score = score
    ...
    def _get_request_score(self, request: Request) -> float:
        score = request.trace_avg_score
        if score is None:
            score = self.trace_scores.get(request.request_id)
        return 0.0 if score is None else score

    def _order_running_by_score(self) -> None:
        if not self.running:
            return
        self.running.sort(
            key=lambda req: (-self._get_request_score(req), req.arrival_time)
        )

    def _reorder_waiting_by_score(self) -> None:
        if not self.waiting:
            return
        ...
        new_queue = create_request_queue(self.policy)
        for req in sorted_requests:
            new_queue.add_request(req)
        self.waiting = new_queue

    def _stop_lowest_running(self) -> Request | None:
        if not self.running:
            return None
    ...
    def update_from_output(
        ...
        final_trace_scores: dict[str, float | None] | None = None,
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            final_score = (
                final_trace_scores.get(req_id)
                if final_trace_scores is not None and finish_reason is not None
                else None
            )
        ...
        if self.pending_pruned_outputs:
            for client_idx, pruned_list in self.pending_pruned_outputs.items():
                if not pruned_list:
                    continue
                eco = engine_core_outputs.get(client_idx)
                if eco is None:
                    eco = EngineCoreOutputs(outputs=[])
                    engine_core_outputs[client_idx] = eco
                eco.outputs.extend(pruned_list)
            self.pending_pruned_outputs.clear()
    ...
    def mark_stop_reason(self, request_ids: Iterable[str], stop_reason: str) -> None:
        """Attach a stop reason to active requests before they are stopped."""
        for req_id in request_ids:
            request = self.requests.get(req_id)
            if request is None or request.is_finished():
                continue
